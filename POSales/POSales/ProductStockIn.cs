    ...
    public partial class ProductStockIn : Form
    {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Select")
            {
                ...
                {
                    MessageBox.Show("Molimo vas unesite zalihe po imenu", stitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    stockIn.txtStockInBy.Focus();
                    this.Dispose();                                        
                }

                if (MessageBox.Show("Dodajte ove zalihe?", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    addStockIn(dgvProduct.Rows[e.RowIndex].Cells[1].Value.ToString());
                    MessageBox.Show("Uspešno dodato", stitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
