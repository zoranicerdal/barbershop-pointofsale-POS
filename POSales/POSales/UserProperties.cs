...
namespace POSales
{
    public partial class UserProperties : Form
    {
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            try
            {
                if ((MessageBox.Show("Da li ste sigurni da želite da promenite svojstva ovog naloga?", "Promeni Svojstva", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes))
                 {
                    cn.Open();
                    ...
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Svojstva naloga su uspešno promenjena!", "Promeni Svojstva", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    account.LoadUser();
                    this.Dispose();
                }
            }
