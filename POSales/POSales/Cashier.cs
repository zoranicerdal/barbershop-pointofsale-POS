...
namespace POSales
{
    public partial class Cashier : Form
    {
        ...
        {
            slide(btnClear);
            if (MessageBox.Show("Želite li da očistite korpu?", "Potvrdi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                cn.Open();
                cm = new SqlCommand("Delete from tbCart where transno like'" + lblTranNo.Text + "'", cn);
                cm.ExecuteNonQuery();
                cn.Close();
                MessageBox.Show("Korpa je uspešno očišćena", "Očisti korpu", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadCart();
            }
        }
        ...
        private void btnLogout_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Nije moguće odjaviti se. Molimo vas otkažite transakciju.", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (MessageBox.Show("Želite se odjaviti?", "Odjava", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Hide();
        ...
        public void GetCartTotal()
        {
            ...
            double vat = sales * 0.20;//VAT: 20% of VAT Payable (Output Tax less Input Tax)
        ...
        public void AddToCart(string _pcode, double _price,int _qty)
        {
            try
            {
                ...
                if (found)
                {
                    ...
                    {
                        MessageBox.Show("Nije moguće nastaviti. Preostala količina zaliha je " + qty, "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    cn.Open();
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Uklonite ovaj proizvod", "Ukloni proizvod", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.ExecuteQuery("Delete from tbCart where id like'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Proizvod je uspešno uklonjen", "Ukloni proizvod", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadCart();
                }
            }
            else if (colName == "colAdd")
            {
                ...
                {
                    MessageBox.Show("Preostala količina zaliha je " + i + "!", "Nema na stanju", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            else if (colName == "colReduce")
            {
                ...
                {
                    MessageBox.Show("Preostala količina u korpi je " + i + "!", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
        }
