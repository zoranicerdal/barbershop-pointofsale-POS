...
namespace POSales
{
    public partial class Supplier : Form
    {
        ...
        private void dgvSupplier_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Da li ste sigurni da želite ukloniti ovog dobavljača?", "POTVRDI", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("Delete from tbSupplier where id like '" + dgvSupplier.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Uspešno ste uklonili dobavljača.", "Uklonite Dobavljača", MessageBoxButtons.OK, MessageBoxIcon.Information);
