...
namespace POSales
{
    public partial class Brand : Form
    {
        ...
        private void dgvBrand_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Da li ste sigurni da želite ukloniti ovaj brend?", "Brisanje Brenda", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbBrand WHERE id LIKE '" + dgvBrand[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Uspešno ste uklonili brend.", "ŠeškoShop", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
