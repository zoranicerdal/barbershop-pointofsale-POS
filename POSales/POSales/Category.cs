...
namespace POSales
{
    public partial class Category : Form
    {
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Da li ste sigurni da želite da uklonite ovu kategoriju?", "Ukloni kategoriju", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbCategory WHERE id LIKE '" + dgvCategory[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Kategorija je uspešno uklonjena.", "ŠeškoShop", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
