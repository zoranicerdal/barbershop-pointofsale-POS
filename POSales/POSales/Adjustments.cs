    ...
    public partial class Adjustments : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbAction.Text == "")
                {
                    MessageBox.Show("Odaberite postupak za dodavanje ili uklanjanje.", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    cbAction.Focus();
                    return;
                }
                ...
                if (txtQty.Text == "")
                {
                    MessageBox.Show("Unesite količinu za dodavanje ili uklanjanje.", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtQty.Focus();
                    return;
                }
                ...
                if (txtRemark.Text == "")
                {
                    MessageBox.Show("Potrebna je napomena za prilagođavanje zaliha.", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtRemark.Focus();
                    return;
                }
                ...
                {
                    MessageBox.Show("Količina raspoloživih zaliha treba da bude veća od unete količine.", "Upozorenje", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (cbAction.Text == "Ukloni zalihe")
                {
                    dbcon.ExecuteQuery("UPDATE tbProduct SET qty = (qty - " + int.Parse(txtQty.Text) + ") WHERE pcode LIKE '" + lblPcode.Text + "'");
                }
                else if (cbAction.Text == "Dodaj zalihe")
                {
                    dbcon.ExecuteQuery("UPDATE tbProduct SET qty = (qty + " + int.Parse(txtQty.Text) + ") WHERE pcode LIKE '" + lblPcode.Text + "'");
                }
                ...
                MessageBox.Show("Uspešno ste prilagodili zalihe.", "Proces završen", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadStock();
                Clear();
